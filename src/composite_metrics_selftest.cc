/*  =========================================================================
    composite_metrics_selftest.c - run selftests
...
test_item_t all_tests [] = {
    DECLARE_TEST(bios_composite_metrics_server_test),
    {0, 0} // Null terminator
};
...
{
    printf ("Running composite-metrics selftests...\n");
    test_item_t *item;
    for (item = all_tests; item->test; item++)
        item->test (verbose);
...
main (int argc, char **argv)
{
    ...
    if (test) {
        printf ("Running composite-metrics selftest '%s'...\n", test->testname);
        test->test (verbose);
    }
    else
        test_runall (verbose);
